...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if(empty($errors) && $schritt2 && empty($errorsUpload)) {
		$uploader -> moveFile();
	// Daten in Datenbank schreiben
	try {
		// prepare - Abfrage vorbereiten (SQL Injection vermeiden)
		$query = $db->prepare("INSERT INTO Posts (title, author, category_id, text, image) VALUES(:title, :author, :category_id, :text, :image)"); 
	...
	header("location: cms_posts.php?adding_success=true"); 
...
<main class="main-content">
	...
	<section class="create-form">
		...
		<div class="create-form__header">
			<p>Neuen Feedback-Beitrag erstellen</p>
		</div>
		...
		<form class="create-form__body" id="form-03" method="POST" enctype="multipart/form-data" action="#">
			...
			<div class="create-form__lines">
				...
				<div class="create-form__line">
					...
					<input class="create-form__input" id="title-03" name="post-title" value="<?php if (isset($_POST['post-title'])) { echo $titlePost; } ?>" autocomplete="off"
						type="text" placeholder="Titel hinzufügen" />
						<?php if (isset($validation->errorsArr['post-title'])): ?>
                    <p class="registration__error"><?= $validation->errorsArr['post-title'][0] ?></p>
                <?php endif; ?>
				</div>

				<div class="create-form__line">
					<label class="create-form__label" for="author-03">Autor(in) *</label>
					<input class="create-form__input" id="author-03" name="author" value="<?php if (isset($_POST['author'])) { echo $author; } ?>" autocomplete="off"
						type="text" placeholder=" Autor(in) hinzufügen" />
						<?php if (isset($validation->errorsArr['author'])): ?>
                    <p class="registration__error"><?= $validation->errorsArr['author'][0] ?></p>
                <?php endif; ?>
				</div>
				...
				<div class="create-form__line">
					...
					<select class="create-form__input" id="category-03" name="category" value="" autocomplete="off"
						type="text">
						<option value="1" <?php if (isset($_POST['category']) && $_POST['category'] == 1) echo 'selected'; ?>>Commercial</option>
						<option value="2" <?php if (isset($_POST['category']) && $_POST['category'] == 2) echo 'selected'; ?>>Privates Wohnen</option>
						<option value="3" <?php if (isset($_POST['category']) && $_POST['category'] == 3) echo 'selected'; ?>>Ferienimmobilien</option>
					</select>
				</div>

				<div class="create-form__line">
					<label class="create-form__label" for="text-03">Text *</label>
					<textarea class="create-form__input" id="text-03" name="text" autocomplete="off"
						type="text" placeholder="Message hinzufügen"><?php if (isset($_POST['text'])) { echo $text; } ?></textarea>
						<?php if (isset($validation->errorsArr['text'])): ?>
                    <p class="registration__error"><?= $validation->errorsArr['text'][0] ?></p>
                <?php endif; ?>
				</div>
				...
				<label class="create-form__label" for="image">Bild anhängen *</label>
                    ...
                    <p class="registration__error"><?= implode('<br>', $errorsUpload) ?></p>
                <?php endif; ?>
			...
			<a href = "cms_posts.php"><button class="create-form__button cancel-button" type="button"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
